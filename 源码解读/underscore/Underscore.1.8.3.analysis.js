...
(function() {
  ...
  //兼容各种代码执行环境，包括浏览器环境和 node 环境，将执行环境的顶层对象赋值
  ...
  //无new构造函数
  var _ = function(obj) {
    //obj 参数为 underscore 函数的实例时，那么 obj instanceof _ 是成立的，直接返回实例
    if (obj instanceof _) return obj;
    //obj 参数为普通值时，返回了一个 underscore 函数的实例
  ...
  eq = function(a, b, aStack, bStack) {
    ...
    // a 和 b是基本数据类型，且值相等时，校验 0 === -0
    // js 语法中，0 === -0成立，1/0 === 1/-0 不成立
    if (a === b) return a !== 0 || 1 / a === 1 / b;
    // `null` or `undefined` only equal to itself (strict comparison).
    if (a == null || b == null) return false;
    // `NaN`s are equivalent, but non-reflexive.
    // 校验 NaN === NaN
    // JS 语法中，NaN != NaN，NaN !== NaN
    if (a !== a) return b !== b;
    // Exhaust primitive checks
    // 前面已经过滤掉了 JS 的基本数据类型的值，如果 a 和 b 的值是复杂数据类型，进入这一步进行深度比较
    var type = typeof a;
    // 过滤掉类型为 undefined 的值，如值为立即执行函数，typeof (function() {})() == 'undefined'
  ...
  deepEq = function(a, b, aStack, bStack) {
    ...
    // a、b 的值都是 underscore 的实例，比较各自的实例参数
    if (a instanceof _) a = a._wrapped;
    if (b instanceof _) b = b._wrapped;
    // Compare `[[Class]]` names.
    // 比较 a、b 的数据类型是否一致，如果数据类型一致，继续往下走
    ...
    // a、b 的数据类型可能是 Array 或者 Object 或者 Function
    var areArrays = className === '[object Array]';
    // 过滤掉 Function 类型值，函数之间比较是不相等的
    if (!areArrays) {
  ...
  // 判断值是否为 Array 数据类型
  // 如果当前执行环境支持 Array.isArray 就优先使用
  ...
  //根据 Object.prototype.toString 精准判断数据类型
  ...
  /**
   * function test() {
   *    // IE 浏览器在控制台上执行 toString.call() 提示 "调用的对象无效"
   *    console.log(Object.prototype.toString(arguments));
   * }
   * test('a') //[object Object] 
   */
  // ie9 以下的浏览器使用 Object.prototype.toString 判断 Argunents 类型，得到的结果都是 [object Object]
  // 需要判断对象是否具有 callee 属性确定是否为 Arguments 类型
  ...
  _.isNaN = function(obj) {
    // 当值为 undefined, {}, 'aaa'字符串时， 全局函数 isNaN 会返回 true
    // 只有值为数字类型并且值为 NaN 时才会返回 true
    return _.isNumber(obj) && isNaN(obj);
  };

  // Is a given value a boolean?
  // 判断值是否为 Boolean 类型，0 和 1 都会返回 false
  ...
  // 判断值是否为 undefined，void 0 代替 undefined
